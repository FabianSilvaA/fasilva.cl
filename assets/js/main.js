...
// Version 6 activity year filter
document.querySelectorAll('.activity-year-btn').forEach(btn=>btn.addEventListener('click',()=>{document.querySelectorAll('.activity-year-btn').forEach(b=>b.classList.remove('active'));document.querySelectorAll('.activity-year').forEach(y=>y.classList.remove('active'));btn.classList.add('active');document.getElementById('activity-'+btn.dataset.activityYear)?.classList.add('active');}));
